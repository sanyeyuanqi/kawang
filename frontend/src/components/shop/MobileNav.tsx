...
export default function MobileNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-1/2 right-auto z-30 flex h-24 w-full max-w-[430px] -translate-x-1/2 items-start justify-around border-t border-[#dfe5ed] bg-white pt-[10px] md:hidden">
      {navItems.map((item) => {
        ...
        return (
          <Link
            key={item.to}
            to={item.to}
            className={"flex h-[60px] w-[82px] flex-col items-center justify-center rounded-[20px] text-center " + (active ? "bg-[#e8f2ff] text-[#0e4beb]" : "text-[#737d8f]")}
          >
